...
- 득표수 상위 노래부터 순차적으로 다음과 같은 방법으로 배정
  1. `꼭 하고싶다`가 2명 혹은 1명일 경우 자동 배정
  2. (1번이 전부 완료후 진행) `꼭 하고싶다`가 3명 이상일 경우 배정된 곡이 적은 인원이 우선순위 배정 (동점일 경우 상의 후 배정)
  3. `꼭 하고싶다`가 전부 결정되면 `시키면한다`를 다시 1번 부터 동일한 방식으로 배정
